function nn = cnn_stn_cluttered_India_init(imsz, use_transformer)
...
% for cluttered MNIST:
  ...
  if use_transformer
    ...
    l_cnv1 = dagnn.Conv('size',[2 2 1 20],'pad',0,'stride',1,'hasBias',true);
    ...
    l_cnv2 = dagnn.Conv('size',[2 2 20 20],'pad',0,'stride',1,'hasBias',true);
    nn.addLayer('l_cnv2', l_cnv2, {'x4'}, {'x5'}, {'lc2f','lc2b'});
    nn.addLayer('l_re2', dagnn.ReLU(), {'x5'}, {'x6'});

    l_fc1 = dagnn.Conv('size',[2,2,20,50],'pad',0,'stride',1,'hasBias',true);
  ...
  % classification net:
  c_cnv1 = dagnn.Conv('size',[2 2 1 32],'pad',0,'stride',1,'hasBias',true);
  nn.addLayer('c_cnv1', c_cnv1, {'xc0'}, {'xc1'}, {'cc1f','cc1b'}); % output dim = 30-6 = 24
  nn.addLayer('c_re1', dagnn.ReLU(), {'xc1'}, {'xc2'});
  c_mp1 = dagnn.Pooling('method', 'max', 'poolSize', [1 1],'pad', 0, 'stride', 1);
  nn.addLayer('c_mp1', c_mp1, {'xc2'}, {'xc3'}); % output dim = 24/2 = 12

  c_cnv2 = dagnn.Conv('size',[3 3 32 48],'pad',0,'stride',1,'hasBias',true);
  ...
  c_fc1 = dagnn.Conv('size',[2 2 48 256],'pad',0,'stride',1,'hasBias',true);
  nn.addLayer('c_fc1', c_fc1, {'xc6'}, {'xc7'}, {'cfcf','cfcb'});
  nn.addLayer('c_re3', dagnn.ReLU(), {'xc7'}, {'xc8'});

  c_fc2 = dagnn.Conv('size',[1 1 256 16],'pad',0,'stride',1,'hasBias',true);
