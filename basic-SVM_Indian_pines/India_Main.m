...
load('PaviaU_gt');
%Indian_Pines标签信息
label=double(paviaU_gt);
image=permute(paviaU,[3,1,2]);
[p m n] = size(image);
...
subplot();
imagesc(Original_image);%colorbar;
map = zeros(15,3);
map=[   255   255   255
        0  107  253
        0  186  253
...
map=map/255.0;
colormap(map);
f2=figure(2);set(f, 'Position',  [0 114 1400 600]);
subplot();
imagesc(Train_image);
map = zeros(15,3);
map=[   255   255   255
...
map=map/255.0;
colormap(map);
f3=figure(3);set(f, 'Position',  [0 114 1400 600]);
subplot();
imagesc(Test_image);
map = zeros(15,3);
map=[   255   255   255
